...
void PlayerController::update(float dt) {
	...
	/*glm::vec3 lightOffset = glm::vec3((glm::normalize(forward - right) - glm::vec3(std::cos(glm::radians(-m_yaw)) * 0.02f, 0.0f, -std::sin(glm::radians(-m_yaw)) * 0.02f)) * 0.9f) - glm::vec3(0.f, 0.3f, 0.f);
	m_candle->getComponent<TransformComponent>()->setTranslation(m_cam->getCameraPosition() + glm::vec3(glm::normalize(forward - right)) - glm::vec3(0.f, 3.4f, 0.f));
	m_candle->getComponent<TransformComponent>()->setRotations(glm::vec3(0.f, glm::radians(-m_yaw + 45), 0.f));
	m_candle->getComponent<LightComponent>()->m_pointLight.setPosition(m_cam->getCameraPosition() + lightOffset);*/
}
