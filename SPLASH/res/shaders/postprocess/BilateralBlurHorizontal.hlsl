				...
				int3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	for(int i = -blurRadius; i <= blurRadius; ++i) {
        ...
        float2 factor;
        factor.r = normpdf(cache[k].r-cache[groupThreadID.x].r, BSIGMA) * bZ * weights[i+blurRadius];
        factor.g = normpdf(cache[k].g-cache[groupThreadID.x].g, BSIGMA) * bZ * weights[i+blurRadius];
        Z += factor;
