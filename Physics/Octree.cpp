...
bool Octree::addEntityRec(BoundingBox* newEntity, Node* currentNode) {
	...
	if (glm::length(isInsideVec) == 0.0f) {
		...
		if (currentNode->childNodes.size() > 0) { //Not leaf node
			...
			if (currentNode->nrOfMeshes < m_softLimitMeshes || currentNode->bb.getHalfSize().x / 2.0f < m_minimumNodeHalfSize) { //Soft limit not reached or smaller nodes are not allowed
				//Add mesh to this node
				currentNode->entities.push_back(newEntity);
				currentNode->nrOfMeshes++;
				entityAdded = true;
...
void Octree::updateRec(Node* currentNode, std::vector<BoundingBox*>* entitiesToReAdd) {
	for (int i = 0; i < currentNode->nrOfMeshes; i++) {
		if (currentNode->entities[i]->getChange()) { //Entity has changed
			...
			if (glm::length(findCornerOutside(currentNode->entities[i], currentNode)) > 0.0f) { //Is no longer contained in this node
...
void Octree::update() {
	...
	for (unsigned int i = 0; i < entitiesToReAdd.size(); i++) {
		addEntity(entitiesToReAdd[i]);
	}
