...
void ModelComponent::renderEditorGui(SailGuiWindow* window) {
	Material* rawMaterial = m_model->getMesh(0)->getMaterial();
	
	PhongMaterial* phongMat = nullptr;
	if (phongMat = dynamic_cast<PhongMaterial*>(rawMaterial)) {
		renderPhongMaterialGui(window, phongMat);
		return;
	}

	PBRMaterial* pbrMat = nullptr;
	if (pbrMat = dynamic_cast<PBRMaterial*>(rawMaterial)) {
...
void ModelComponent::renderPhongMaterialGui(SailGuiWindow* window, PhongMaterial* material) {
...
void ModelComponent::renderPBRMaterialGui(SailGuiWindow* window, PBRMaterial* material) {
