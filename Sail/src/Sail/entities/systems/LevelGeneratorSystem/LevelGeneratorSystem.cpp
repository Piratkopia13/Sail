	...
	for (auto e : entities) {
		...
		for (int i = 0; i < amount; i++) {
			...
			if (rand() % 100 < map->clutterModifier) {
				...
				if(rand()%2==0){
					//add on first half
					{
						float posx = (rand() % 40) / 100.f - 0.7f;
						float posy = (rand() % 40) / 100.f - 0.2f;
					...
					float posx = (rand() % 140) / 100.f - 0.7f;
					float posy = (rand() % 40) / 100.f - 0.2f;
		...
		for (int i = 0; i < amount; i++) {
			...
			if (rand() % 100 < map->clutterModifier) {
				...
				clutter cS;
				cS.posx = posx * cos(cM.rot) - posy * sin(cM.rot)+cM.posx;
