#include "BrightnessCutoffStage.h"

BrightnessCutoffStage::BrightnessCutoffStage(UINT width, UINT height, Model* fullscreenQuad)
	: PostProcessStage(width, height, fullscreenQuad)
{
	// Set up constant buffer
	//CBuffer cbuffer = { 0.7f };
	//m_cBuffer = std::unique_ptr<ShaderComponent::ConstantBuffer>(new ShaderComponent::ConstantBuffer(&cbuffer, sizeof(cbuffer)));
	...
	auto psBlob = ShaderSet::compileShader(L"postprocess/BrightnessCutoffShader.hlsl", "PSMain", "ps_5_0");
...
BrightnessCutoffStage::~BrightnessCutoffStage() {
}

void BrightnessCutoffStage::run(RenderableTexture& inputTexture) {
	// TODO: fix
