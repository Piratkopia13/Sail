...
const Shader::ComputeSettings* FXAAShader::getComputeSettings() const {
	return &m_settings;
}

std::pair<std::string, void*> FXAAShader::getComputeInputForIndex(Shader::ComputeShaderInput& input, unsigned int index) {
...
RenderableTexture* FXAAShader::getComputeOutputForIndex(Shader::ComputeShaderOutput& output, unsigned int index) {
...
Shader::ComputeShaderOutput* FXAAShader::getComputeOutput() {
	return m_output.get();
}

FXAAShader::FXAAShader()
	: Shader("postprocess/FXAAShader.hlsl")
{
	...
	m_settings.threadGroupXScale = 1.f / 1.f;
